...
export class SearchFormComponent implements OnInit {
  ...
  /** 「公開年代」セレクトボックス定義 */
  public searchPublishedAges: Array<{ age: number; }>;
  
  /** 映画の始まりの年 : 世界初の映画は 1895 年とされているので 1890 を開始年にする */
  private readonly startAge: number = 1890;
  ...
  public ngOnInit(): void {
    ...
    // private フィールドの宣言より前だと正常に動作しないためココでやる
    this.searchPublishedAges = this.createSearchPublishedAges();
  }
  ...
  public isInvalidPublishedYear(): boolean {
    ...
    return searchTextNumber < this.startAge || searchTextNumber > 2100;  // 映画の始まりの年以前、自分が死んでそうな未来の値は非活性にする
  }
  ...
  public onSearch(): void {
    ...
    const searchText   = this.trimValue(this.searchForm.value.searchText);
    const findParams = searchText ? { targetColumn, searchText } : undefined;  // 検索文字列がある時だけ値を渡す
    this.onSubmit.emit(findParams);
  }
